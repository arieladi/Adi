...
Two Claude Code agents work on `VST-ADI`, on different machines and different
...
|---|---|---|---|
| `VST-ADI/collab/**`, `VST-ADI/docs/**` | win | `main` (bootstrap) | 2026-09-18 |

## Before you start work, every time

```bash
git -C <Adi repo> checkout main && git pull
cat VST-ADI/collab/README.md      # claims may have changed
cat VST-ADI/collab/<other>.md     # what they did since you last looked
cat VST-ADI/docs/DECISIONS.md     # settled questions; do not re-litigate
```
...
Two repositories, and they are **not** nested in the git sense:

```
arieladi/Adi  (monorepo, both agents have it)
└── VST-ADI/
    ├── ARCHITECTURE.md   ┐
    ├── docs/             │ tracked in Adi — visible to both agents
    ├── collab/           │
    ├── tools/            ┘
    └── vital/            ← SEPARATE REPO, gitignored by Adi
```
...
ADR-0011; this is the open question.

Do not "fix" this by `git add`-ing `VST-ADI/vital` into the monorepo. The ignore
rule in `Adi/.gitignore` is deliberate and stops it becoming a stray gitlink.
...
& $msbuild VST-ADI\vital\standalone\builds\vs19\Vial.sln `
    /p:Configuration=Release /p:Platform=x64 /p:PlatformToolset=v143 /m

# VST3 plugin
& $msbuild VST-ADI\vital\plugin\builds\vs19\Vial.sln `
    /p:Configuration=Release /p:Platform=x64 /p:PlatformToolset=v143 /m
```
...
- `python VST-ADI/tools/extract_schema.py` still reports 794 parameters,
...
- **Keep the fork delta small and legible.** `git -C VST-ADI/vital diff
