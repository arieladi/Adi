...
└── adi-vst/                 this project
    ...
    ├── tools/               our Python
    │   ├── extract_schema.py
    │   ├── find_phantom_params.py
    │   ├── validator/       headless VST3 smoke test
    │   └── out/*.json       generated schema (committed deliberately)
    └── vital/               fork of mtytel/vital — ITS OWN GIT REPO, gitignored
```
...
`vital/` is a separate git repository, ignored by the monorepo (see its
