...
# The suites (ADR-0171, ADR-0173): the kept algorithms ship as ten CLAP plug-ins, one
...
def write_catalogue(effects, out, pin):
    ...
    lines = [
        ...
        f"Consolidated is built from. **{len(kept)} are kept**, as the algorithms of ten "
...
# ADR-0173: the console systems and channel colours are not a plug-in. They
# are the mixer's native group summing, compiled into the engine.
SUMMING_CATEGORIES = {"Consoles", "Tone Color"}
