...
registry Airwindows Consolidated is built from) and writes three files:
  ...
  plugins/airwindows/SUITES.md           each suite plug-in's algorithms, in its order
...
def write_suites(effects, out, pin):
    """Each suite's algorithms, in the order of its Algorithm parameter."""
    ...
    lines = [
        ...
        "<!-- Generated by tools/airwindows_catalogue.py; edit that, not this. -->",
        "",
        f"From airwin2rack {pin}. Each suite is one CLAP plug-in (ADR-0171, ADR-0173); its "
        "Algorithm parameter chooses what plays, in the order below (the parameter's value is "
        "the row's index, from 0). The console systems are not here: they are the mixer's "
        "native group summing (ADR-0174). *Auto gain* marks the algorithms whose default is on; "
        "a suite's own switch starts on where most of its algorithms' is.",
        "",
    ]
    for i, (key, name, _) in enumerate(SUITES):
        members = [e for j, e in rows if j == i]
        lines += [f"## {name} ({len(members)})", "", f"CLAP id `com.adi.airwindows.{key}`.", "",
                  "| # | Algorithm | What it does | Auto gain |", "|---:|---|---|---|"]
        for n, e in enumerate(members):
            desc = e["desc"].replace("|", "\\|")
            lines.append(f"| {n} | {e['name']} | {desc} | {'on' if e['auto_gain'] else ''} |")
        lines.append("")
    summing = [e for e in effects if e["keep"] and e["category"] in SUMMING_CATEGORIES]
    lines += [f"## Not a plug-in: the mixer's group summing ({len(summing)})", "",
              "Compiled into the engine (ADR-0174); EveryConsole is compiled in neither, "
              "because it cannot select a system at this commit.", ""]
    lines += [f"- {e['name']}: {e['desc']}" for e in summing]
    (out / "SUITES.md").write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
...
def main():
    ...
    write_suites(effects, args.out, pin)
