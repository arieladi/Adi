...
**3997 checks across 41 suites**, green on 7 ABIs. Nothing is frozen.
...
| [`docs/format/schema.sql`](docs/format/schema.sql) | Normative DDL. Executable, and verified on every change. 42 tables. |
...
| [`docs/DECISIONS.md`](docs/DECISIONS.md) | Decision log. Append-only. 154 entries. |
...
| [`tools/validate_ops.py`](tools/validate_ops.py) | Checks the 163-op catalogue: unique names, inverses, scope rules, coalescing, and that the prose count matches the tables. |
...
| **3** | The op vocabulary: every op type, payload, inverse | **done** — 163 ops, `docs/OPS.md` |
