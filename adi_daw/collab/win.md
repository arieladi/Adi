...
**Open missions** (prompts in `collab/prompts/2026-09-24d-*.md`):
- linux, ADR-0155: MIDI clips into instruments, no hanging notes; and
  ADR-0157 d2, clip playback to 768 kHz with time-based read-ahead.
- cloud, ADR-0156: one decoder into the cache (FLAC, AIFF, MP3, Ogg), then
  the whole settings registry from `docs/SETTINGS-CATALOGUE.md`.

**Waiting on mac** (back Saturday 2026-09-26):
- CI: `adi_play` in the JUCE job (`adi_play <demo> --render 4` needs no audio
  device), expect `JUCE_ASIO` on Windows, an MSVC `ADI_WERROR=ON` leg, the GCC
  TSan leg, the Clang TSan and `test_device` conflict, the benchmark job.
- CLAP: a per-instance `clap_host_t` (C4, ADR-0123), then re-reading the
  parameter list on `RESCAN_ALL`, answering `request_flush`, a state signal
  attributable to one plug-in (ADR-0142), and a CLAP route override
  (ADR-0149).
- `DeviceNode` bypass flags as atomics; a review of the step-6 additions
  in `device_host`.
- UI (step 7), with its core already on main: the device view's panel
  (`panel::resolve`, `panel::search`, `ParamOps::lastTouched`,
  `device.setPanel`; ADR-0150, ADR-0154); the Settings window over
  `src/adi/settings/`; the changeset's Apply (ADR-0148); the Pd editor on
  plugdata (ADR-0145 d8); the library browser over `src/adi/library/`, with
  the macOS volume adapter (ADR-0147); `UI-ARCHITECTURE.md` for
  ADR-0101 and ADR-0112.
- CoreAudio at 88.2 to 768 kHz where a device offers it (ADR-0157).

**Waiting on the director:** `docs/AWAITING.md` rows 1-4 (AVX-512 on the
i9 laptop, ASIO, hearing a project's clips, high rates on real hardware).
`tools/make_demo_project.py` makes the project for the listening rows.

**win's standing work:** build every merge with MSVC `/WX` (after
`tools/fetch_external.sh --build-only`) and run `tools/test_all.sh` on it.
Nothing else of mine is open. Candidates when asked: a native Windows
volume-identity adapter (the marker file already works, ADR-0147), and the
CLAP items above once mac lands C4.

**What is on win's machine and not in the repo, on purpose:** the Word
references (`reference/DOCS/WORD/`: Master Reference v0.7, Settings Reference
v0.4) and their generator in `_build/`. The `reference/` tree is git-ignored
because it holds the DAW manuals, excerpts from them and their screenshots.
`docs/SETTINGS-CATALOGUE.md` is the part that can be public, regenerated by
`tools/export_settings_catalogue.py`. Machine: Ryzen 7 5700X3D (AVX2, no
AVX-512), Visual Studio 2022 Community, no ASIO driver installed. Build trees:
`build/` (core, MSVC) and `build-juce/` (`tools/build-juce.bat`).
