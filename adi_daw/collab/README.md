...
| `src/adi/engine/clip_*` (not the worker's media-open call, which is cloud's), `src/adi/engine/transport.*`, new `src/adi/engine/midi_*`, `tests/test_clip_playback.cpp`, `tests/test_midi_clips.cpp` (new), own `CMakeLists.txt` targets and README headline; lent: `src/adi/engine/session.*`, `graph.*`, `realize.*` | linux | `linux/midi-clips` | 2026-09-24 |
